...
ViewGroup的事件分发过程（dispatchTouchEvent方法）
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        if (onFilterTouchEventForSecurity(ev)) {
			...
			//当ACTION_DOWN事件，ViewGroup总会调用onInterceptTouchEvent;当事件由子View处理，mFirstTouchTarget != null
            ...
            if (actionMasked == MotionEvent.ACTION_DOWN
				...
				//子View可以调用getParent().requestDisallowInterceptTouchEvent(boolean disallowIntercept)方法设置这个flag，来处理拦截事件(前提onInterceptTouchEvent(ev)方法不拦截ACTION_DOWN事件)
...
1处代码，li.mOnTouchListener != null并且onTouch方法返回true，表示事件被消费；onTouchEvent(event)。
